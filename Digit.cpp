...
void Digit::Morph2() {
  ...
  for (int i = 0; i <= segWidth; i++)
  {
    ...
    vTaskDelay(pdMS_TO_TICKS(animSpeed));
  }
}
...
void Digit::Morph7() {
  ...
  for (int i = 0; i <= (segWidth + 1); i++)
  {
    ...
    drawLine(0 + i - 1, 1, 0 + i - 1, segHeight, black);//display.showBuffer();
    drawLine(0 + i, 1, 0 + i, segHeight, _color);//display.showBuffer();

    // Move F left to right
    drawLine(0 + i - 1, segHeight * 2 + 1, 0 + i - 1, segHeight + 2, black);//display.showBuffer();
    drawLine(0 + i, segHeight * 2 + 1, 0 + i, segHeight + 2, _color);//display.showBuffer();

    // Erase D and G gradually
    drawPixel(1 + i, 0, black); // D
    drawPixel(1 + i, segHeight + 1, black);// display.showBuffer();
    vTaskDelay(pdMS_TO_TICKS(animSpeed));
  }
}
